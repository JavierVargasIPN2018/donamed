                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    <TabsList>
                        ...
                        <TabsTrigger value="disponible">
                            Activas ({medications.filter((d) => d.status === "disponible").length})
                        </TabsTrigger>
                        <TabsTrigger value="entregado">
                            Completadas ({medications.filter((d) => d.status === "entregado").length})
                        </TabsTrigger>
                    </TabsList>
                        ...
                        {filteredMedications.length === 0 ? (
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                {filteredMedications.map((donation) => {
                                    const config = statusConfig[donation.status as keyof typeof statusConfig];
                                        ...
                                        <Card key={donation.id} className="hover:shadow-md transition-shadow">
                                            <CardHeader>
                                                <div className="flex items-start justify-between">
                                                    <div className="space-y-1">
                                                        ...
                                                        <CardDescription>
                                                            {donation.dosage} • {donation.quantity} {donation.presentation || "unidades"}
                                                        </CardDescription>
                                                    </div>
                                            ...
                                            <CardContent className="space-y-4">
                                                ...
                                                <div className="text-sm text-muted-foreground">
                                                    Publicada hace{" "}
                                                    {Math.floor((new Date().getTime() - new Date(donation.createdAt).getTime()) / (1000 * 60 * 60 * 24))}{" "}
                                                    días
                                                </div>
                                                ...
                                                {donation.status === "disponible" && (
                                                    <div className="flex items-center justify-between pt-2 border-t">
                                                        <span className="text-sm font-medium">
                                                            {donation.requestsCount} solicitud{donation.requestsCount !== 1 && "es"}
                                                        </span>
                                                        <Button
                                                            size="sm"
                                                            variant="outline"
                            ...
                            <div className="flex items-center justify-between">
                                <h2 className="text-xl font-semibold">
                                    ...
                                    {medications.find((m) => m.id === selectedDonation)?.brand ||
                                        medications.find((m) => m.id === selectedDonation)?.activeSubstance}
                                </h2>
                                <Button variant="ghost" size="sm" onClick={() => setSelectedDonation(null)}>
                                    Cerrar
                                </Button>
                            </div>
